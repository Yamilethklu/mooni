...
export default function InteractiveSimulation() {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      if (active.estado === 'en_camino' && elapsed >= 30000) actualizarEstado(active.id, 'en_viaje');
      if (active.estado === 'en_viaje' && elapsed >= 60000) actualizarEstado(active.id, 'finalizado');
    }, 1000);
  ...
  async function submit() {
    ...
    if (step === 1) { if (!/^\+?[0-9 ()-]{8,20}$/.test(value)) { setError('Escribe un teléfono válido de 8 a 20 caracteres.'); return; } setPhone(value); registrarPasajero(name,value); setStep(2); setInput(''); return; }
  ...
  function requestRide(amount: number) {
    if (!route) return;
    const v: Viaje = { nombrePasajero:name, id:crypto.randomUUID().slice(0,8).toUpperCase(), pasajero:phone, origen:origin, destino:destination, km:route.km, min:route.min, precioSugerido:calculateTarifa(route.km,route.min), oferta:amount, estado:'buscando_conductor', createdAt:new Date().toISOString() };
    agregarViaje(v); setCurrentId(v.id); setStep(6);
  ...
  function acceptCounter() {
    ...
    asignarConductor(active.id, counter);
    setDriverMessage(`Tú: Acepto $${counter.toFixed(2)} MXN. Ahmed: ¡Perfecto, voy por ti!`);
    setCounter(null);
  }
  function reset() {
    setStep(0); setName(''); setOrigin(''); setDestination(''); setRoute(null); setPhone(''); setOffer(''); setCurrentId(null); setCounter(null); setDriverMessage('');
  }
